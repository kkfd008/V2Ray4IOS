import SwiftUI

/// VLESS 配置视图 — 紫色主题
struct VLESSConfigView: View {
    let onSave: (ServerConfig) -> Void
    @Environment(\.dismiss) private var dismiss

    @State private var name = ""
    @State private var address = ""
    @State private var port = "443"
    @State private var uuid = ""
    @State private var flow = "none"
    @State private var encryption = "none"
    @State private var transport: ServerConfig.TransportType = .ws
    @State private var realityEnabled = true

    let accentColor = Color(hex: "c882f0")

    var body: some View {
        ZStack {
            Color(hex: "0b0f14").ignoresSafeArea()

            ScrollView {
                VStack(spacing: 0) {
                    HStack {
                        Button("← 返回") { dismiss() }
                            .font(.system(size: 14, weight: .medium))
                            .foregroundColor(Color(hex: "00e5a0"))
                        Spacer()
                        Text("VLESS 配置")
                            .font(.system(size: 20, weight: .bold))
                            .foregroundColor(Color(hex: "e4e8ee"))
                        Spacer().frame(width: 30)
                    }
                    .padding(.bottom, 16)

                    ConfigSection(title: "目标服务器", accent: accentColor) {
                        ConfigField(label: "备注", text: $name, placeholder: "例如：JP Tokyo")
                        ConfigRow {
                            ConfigField(label: "地址", text: $address, placeholder: "server.com")
                            ConfigField(label: "端口", text: $port, placeholder: "443")
                        }
                    }

                    ConfigSection(title: "身份凭证", accent: accentColor) {
                        ConfigField(label: "UUID", text: $uuid, placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
                        ConfigRow {
                            ConfigPicker(label: "Flow", selection: $flow, options: ["none", "xtls-rprx-vision"])
                            ConfigPicker(label: "Encryption", selection: $encryption, options: ["none"])
                        }
                    }

                    ConfigSection(title: "传输与安全", accent: accentColor) {
                        ConfigPicker(label: "传输协议", selection: transportBinding, options: ["ws", "tcp", "grpc", "h2"])
                        ToggleRow(label: "Reality", isOn: $realityEnabled)
                    }

                    Button(action: save) {
                        Text("保存配置")
                            .font(.system(size: 15, weight: .bold))
                            .foregroundColor(.black)
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 14)
                            .background(accentColor)
                            .clipShape(RoundedRectangle(cornerRadius: 10))
                    }
                    .padding(.top, 4)
                }
                .padding(.horizontal, 20)
                .padding(.bottom, 20)
            }
        }
    }

    private var transportBinding: Binding<String> {
        Binding(get: { transport.rawValue }, set: { transport = ServerConfig.TransportType(rawValue: $0) ?? .tcp })
    }

    private func save() {
        let config = ServerConfig(
            name: name,
            address: address,
            port: Int(port) ?? 443,
            protocolType: .vless,
            protocolSpecific: .vless(.init(uuid: uuid, flow: flow, encryption: encryption, realityEnabled: realityEnabled)),
            transport: transport
        )
        onSave(config)
    }
}